...
class LocalOptimizationLinearConstraints(
    KinematicModel,
    ObjMinimizeActivation,
    ConstraintAccelerationTarget,
    ResidualForces,
    ...
    def upd_model_kinematics(self, frame):
        super().upd_model_kinematics(frame)
        self._prepare_constraints(np.ones(self.n_muscles))

    def _prepare_constraints(self, activation):
        self.model.realizeVelocity(self.state)
    ...
    def objective(self, x):
        super().objective(x * self.previous_activation)

    def gradient(self, x):
        super().gradient(x * self.previous_activation)
