...
class KinematicModel:
    ...
    def __dispatch_kinematics(self):
        ...
        if self.should_low_pass_filter:
            self.__data_storage.lowpassFIR(
                self.low_pass_order, self.low_pass_frequency
            )  # TODO read value in xml file or class parameters  MICKAEL removed filter
        self.gen_coord_function = osim.GCVSplineSet(
            5, self.__data_storage
        )  # TODO read value in xml file or class parameters
            ...
            for idx_q in range(self.__data_storage.getStateVector(frame).getSize()):
                q.append(
                    self.gen_coord_function.evaluate(
                        idx_q, 0, self.__data_storage.getStateVector(frame).getTime()
                    )
                )
                qdot.append(
                    self.gen_coord_function.evaluate(
                        idx_q, 1, self.__data_storage.getStateVector(frame).getTime()
                    )
                )
                qddot.append(
                    self.gen_coord_function.evaluate(
                        idx_q, 2, self.__data_storage.getStateVector(frame).getTime()
                    )
                )
            self.all_q.append(q)
...
class ClassicalStaticOptimization(
    ...
    def __init__(
        ...
        model_path,
        mot_path,
        low_pass_filter_param=None,
        activation_exponent=2,
        residual_actuator_xml_path=None,
        external_loads_xml_path=None,
    ):
        KinematicModel.__init__(self, model_path, mot_path, low_pass_filter_param)
        ObjMinimizeActivation.__init__(self, activation_exponent)
        ResidualForces.__init__(self, residual_actuator_xml_path)
        ExternalForces.__init__(self, external_loads_xml_path)
...
class ClassicalOptimizationLinearConstraints(
    ...
    def __init__(
        ...
        ResidualForces.__init__(
            self, residual_actuator_xml_path=residual_actuator_xml_path
        )
